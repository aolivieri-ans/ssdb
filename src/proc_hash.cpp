...
int proc_multi_hincr(NetworkServer *net, Link *link, const Request &req, Response *resp)
{
	SSDBServer *serv = (SSDBServer *)net->data;
	if (req.size() < 4 || req.size() % 2 != 0)
	{
	...
	else
	{
		...
		const Bytes &name = req[1];
		std::vector<Bytes>::const_iterator it = req.begin() + 2;
		Transaction trans(serv->ssdb->binlogs);
		int ret;
		for (; it != req.end(); it += 2)
		{
			const Bytes &key = *it;
			const Bytes &byStr = *(it + 1);
			// TODO andoli
			int64_t by = str_to_int64(byStr.String());
			if (errno != 0)
			{
				resp->push_back("error");
				return 0;
			...
			int64_t newval = -1;
			ret = serv->ssdb->hincr_notransaction(name, key, by, &newval);
			if (ret == -1)
			{
				resp->push_back("error");
				return 0;
