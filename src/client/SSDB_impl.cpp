...
namespace ssdb
{
	...
	inline static Status _read_list(const std::vector<std::string> *resp, std::vector<std::string> *ret)
	{
		Status s(resp);
		if (s.ok())
		{
	...
	inline static Status _read_int64(const std::vector<std::string> *resp, int64_t *ret)
	{
		Status s(resp);
		if (s.ok())
		{
	...
	inline static Status _read_str(const std::vector<std::string> *resp, std::string *ret)
	{
		Status s(resp);
		if (s.ok())
		{
	...
	Status ClientImpl::multi_hincr(const std::string &name, const std::map<std::string, std::string> &kvs)
	{
		const std::vector<std::string> *resp;
		std::vector<std::string> list;
		for (std::map<std::string, std::string>::const_iterator it = kvs.begin();
		...
		resp = this->request("multi_hincr", name, list);
		Status s(resp);
		return s;
	}
